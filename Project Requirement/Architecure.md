...
┌────────────────────────────────────────────────────────┐
│                      FLUTTER UI                        │
│   (Material 3 Layout / State Management: Provider)     │
└───────────┬────────────────────────────────┬───────────┘
│                                │
Write Operations                 Real-time Listeners
(Transactions / Sets)            (Document Streams)
│                                │
▼                                ▼
┌────────────────────────────────────────────────────────┐
│                    FIREBASE ENGINE                     │
...
## 2. Data Flow & State Mechanics
*   **Command Pattern Execution:** Write operations are dispatched through decoupled services (e.g., `FirestoreService`). Cross-document mutations utilize atomic transactions (`runTransaction`) to fulfill absolute consistency constraints.
*   **Reactive UI Updates:** The tenant waiting room sets up a dedicated stream observer (`snapshots().listen()`) tracking changes to their document. When an approval write succeeds, the listener hooks the state adjustment instantly, self-terminates safely to avoid resource leakage, and pushes a clean navigation route shift.
*   **Owner Screen Auto-Refresh:** Owner screens utilize explicit auto-refresh interval loops (15-second cycles) coupled with direct programmatic refreshes triggered immediately following local operational execution to keep the dashboard updated without heavy streaming loops.